...
angular.module('lmisChromeApp')
  ...
  .controller('AddProductCtrl', function($scope, storageService, $location) {
    ...
    storageService.get(storageService.PRODUCT_CATEGORY).then(function(
      productCategories) {
    ...
    $scope.saveProduct = function() {
      ...
      if (Object.keys($scope.product).length > 0) {
        storageService.insert(storageService.PRODUCT, $scope.product).then(
          function(bool) {
            if (bool) {
              $scope.setMessage({
                message: 'Data saved ',
  ...
  .controller('AddProductItemCtrl', function($scope, storageService, $location) {
    ...
    storageService.get(storageService.PRODUCT_PRESENTATION).then(function(
      presentationList) {
    ...
    storageService.get(storageService.MODE_OF_ADMINISTRATION).then(function(
      modeOfAdministrationList) {
      $scope.modes = modeOfAdministrationList;
    });

    storageService.get(storageService.PRODUCT_FORMULATION).then(function(
      formulationList) {
    ...
    */
    $scope.save = function() {
      storageService.insert(storageService.PRODUCT_ITEM, $scope.productItem).then(
        function() {
   ...
   * storage.
   */
  .controller('ProductItemListCtrl', function($scope, storageService,
    visualMarkerService, utility, $filter, ngTableParams) {
    ...
    utility.loadTableObject(storageService.PRODUCT_PRESENTATION).then(function(
      productPresentation) {
      $scope.presentations = productPresentation;
    });

    utility.loadTableObject(storageService.PRODUCT_FORMULATION).then(function(
      productFormulation) {
      $scope.productFormulation = productFormulation;
    });

    utility.loadTableObject(storageService.MODE_OF_ADMINISTRATION).then(
      function(modesOfAdmin) {
  ...
  .controller('ProgramsProductsCtrl', function($scope, storageService) {
    ...
    storageService.loadTableObject(storageService.PRODUCT).then(function(
      products) {
   ...
   */
  .controller('programProductFormCtrl', function($scope, storageService,
    $location) {
