...
angular.module('lmisChromeApp')
  .controller('cceCtrl', function($scope, storageService, utility, $filter, ngTableParams) {
    ...
    utility.loadTableObject(storageService.STORAGE_LOCATION_TYPE).then(function(
      cceTypes) {
    ...
    utility.loadTableObject(storageService.STORAGE_LOCATION).then(function(
      cceList) {
  ...
  /**
   * This controller is used to save CCE record to local storage or remote DB
   * via REST API.
   */
   ...
   */
  .controller('cceProblemLogMainCtrl', function($scope, storageService, utility,
    $filter, ngTableParams) {

    storageService.get(storageService.STORAGE_LOCATION_PROBLEM).then(function(
      data) {
      // Table defaults
      var params = {
        page: 1,
  ...
  /**
   * AddProblemLogCtrl is used for adding problems logs to a selected Cold Chain
   * Equipment.
   */
  ...
  .controller('cceTemperatureLogMainCtrl', function($scope, storageService, $filter, ngTableParams, utility) {
    ...
    storageService.get(storageService.STORAGE_LOCATION_TEMPERATURE).then(
      function(data) {
