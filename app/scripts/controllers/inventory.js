...
angular.module('lmisChromeApp')
  ...
  .controller('StockRecordsFormCtrl', function($scope, $location, storageService) {
    ...
    storageService.get('monthly_stock_record').then(function(data) {
      if (Object.prototype.toString.call(data) === '[object Array]') {
        if (data.length > 0) {
          storageService.loadTableObject('monthly_stock_record').then(
            function(data) {
    ...
    $scope.stock_records.program = $scope.facility_programs[$scope.facility_uuid]
      .program;
   ...
   */
  .controller('inventoryMainCtrl', function($scope, storageService, $filter, ngTableParams, utility, visualMarkerService) {
    var date_day = [];
    for (var i = 1; i < 32; i++) {
      date_day.push(i);
    }
    ...
    storageService.get(storageService.INVENTORY).then(function(data) {
      ...
      storageService.loadTableObject(storageService.STORAGE_LOCATION).then(
        function(data) {
          $scope.cceList = data;
        });
